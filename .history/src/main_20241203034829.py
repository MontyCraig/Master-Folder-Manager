...
def select_volume() -> Optional[Path]:
    ...
    try:
        ...
        choice = Prompt.ask(
            "Select volume (or Enter to cancel)",
...
def select_directory(start_path: Optional[Path] = None) -> Optional[Path]:
    ...
    try:
        ...
        while True:
            ...
            choice = Prompt.ask(
                "Select option",
                choices=[str(i) for i in range(1, len(dirs) + 1)] + ['p', 's', 'v', 'c']
            )
...
def display_main_menu() -> str:
    ...
    console.print(Panel.fit(
        "[bold cyan]Enhanced Folder Manager[/bold cyan]\n"
...
def organize_files_interactive():
    ...
    try:
        ...
        # Confirm master directory
        config = settings.load_config()
        master = Path(config["master_folder_root"])
        
        rprint(f"\nFiles will be organized into: {master}")
        if not Confirm.ask("Continue with this location?"):
            rprint("\n[bold cyan]Select master directory:[/bold cyan]")
            master = select_directory()
            if not master:
                return
