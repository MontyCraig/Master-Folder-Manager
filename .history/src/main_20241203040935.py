...
def select_directory(start_path: Optional[Path] = None) -> Optional[Path]:
    ...
    try:
        ...
        while True:
            # Show current volume and directory
            current_vol = current.anchor or Path.cwd().anchor
            rprint(f"\n[bold cyan]Current Volume:[/bold cyan] {current_vol}")
            rprint(f"[bold cyan]Current Directory:[/bold cyan] {current}")
            
            # Show directory contents
            items = list(current.iterdir())
            dirs = [d for d in items if d.is_dir()]
            
            table = Table(title=f"Contents of {current}")
            table.add_column("Index", style="cyan", justify="right")
            ...
            if choice == 'q':
                ...
                if current.parent == current:  # At root
                    rprint("[yellow]Already at root directory[/yellow]")
                else:
...
def organize_files_interactive():
    ...
    try:
        ...
        source_vol = source.anchor
        rprint(f"\n[green]Selected Source:[/green]")
        rprint(f"Volume: {source_vol}")
        rprint(f"Path: {source}")
        ...
        master_vol = master.anchor
        rprint(f"\n[green]Selected Master Folder:[/green]")
        rprint(f"Volume: {master_vol}")
        rprint(f"Path: {master}")
        
        # Confirm operation
        move_files = Confirm.ask("Move files? (No will copy them)")
        
        # Confirm final operation with volume information
        if not Confirm.ask(
            f"\nReady to {'move' if move_files else 'copy'} files:\n"
            f"From: {source_vol} - {source}\n"
            f"To: {master_vol} - {master}\n"
            "Continue?"
        ):
            return
